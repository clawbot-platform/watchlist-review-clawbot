...
func TestServiceGenerateSkippedWhenNotConfigured(t *testing.T) {
	...
	note := service.Generate(context.Background(), nil, nil, nil, nil, nil)
...
func TestServiceGenerateReturnsNote(t *testing.T) {
	...
	note := service.Generate(context.Background(), alert, fx, score, nil, nil)
...
func TestBuildPrompt(t *testing.T) {
	...
	score := scoring.Evaluate(alert, fx)
	prompt, err := BuildPrompt(PromptInput{Alert: alert, Features: fx, Score: score})
	if err != nil {
